...
required_literals = [
    ...
    ("replace(/-([a-z])/g", "clickable audit must normalize hyphenated option aliases"),
    ("const optionName = name.replace(/-([a-z])/g", "clickable audit must read required hyphenated options after alias normalization"),
    ("parseAllowedIndexes(options.allowIndexes)", "interactive audit must parse explicit candidate indexes"),
    ("parseAllowedMatchers(options.allowLabels", "interactive audit must support fuzzy label allowlists"),
    ("fuzzyLabelMatches(label, allowed)", "interactive fuzzy labels must not match broad aggregate containers"),
    ("resolveAllowedIndexes(candidates", "interactive audit must resolve explicit and fuzzy allowlists after inventory"),
...
required_risk_terms = [
    ...
    "建立", "create", "新增", "復原", "還原", "undo", "restore", "revert", "套用", "apply",
